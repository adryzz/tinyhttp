...
use crate::{config::StaticPage, error::Error, status::StatusCode, utils, request::HttpVersion};
...
impl<'a, 'b> HttpWriter<'a, 'b, Start> {
    pub(crate) fn new(socket: &'a mut TcpWriter<'b>, version: HttpVersion) -> HttpWriter<'a, 'b, Start> {
...
impl<'a, 'b> HttpWriter<'a, 'b, Headers> {
    ...
    pub async fn body_chunked(mut self, length: usize, content_type: &str) -> Result<ChunkedHttpWriter<'a, 'b>, Error> {
        let mut buf = utils::USizeStrBuf::new();
        self = self.header("Content-Type", content_type).await?
        .header("Content-Length", buf.stringify(length)).await?;
